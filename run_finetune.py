...
def main():
    ...
    if model.config.decoder_start_token_id is None:
        raise ValueError(
            f"Make sure that `config.decoder_start_token_id` is correctly defined."
        )
    ...
    if hasattr(model.generation_config, "is_multilingual") and model.generation_config.is_multilingual:
        ...
        model.generation_config.update(**{"language": data_args.language, "task": data_args.task})
    ...
    if training_args.generation_num_beams is not None:
        ...
        num_beams = getattr(model.generation_config, "num_beams", 1)
    ...
    for epoch in range(epochs_trained, num_epochs):
        ...
        for batch in train_dataloader:
            with accelerator.accumulate(model):
                ...
                accelerator.backward(model_outputs.loss)
                train_metric = {"loss": model_outputs.loss.detach()}
